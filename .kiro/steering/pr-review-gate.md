...
2. **Eye 掃描** 對該分支的實際畫面（桌面 + 手機）執行視覺驗收，檢查：
   ...
   - 無 overflow / 無空白頁
3. **/codex-review 對抗審** 執行對抗式程式碼審查，修正所有 findings 並重跑受影響的 local gates。
...
- 所有 review findings 已解決。
- Eye 掃描證據記錄在 PR body 或 comment。
- `/codex-review` 無未修正的 HIGH/MEDIUM finding。
- security 相關修改需 harper (CISO) + gray (CPO) review；cost 相關修改至少需 harper review。
- production deployment 只走明確 release workflow；issue-development sweep 不 merge、不 release、不 deploy。
