...
- fix: 手動分析與背景自動工作分離，並在 durable pipeline 的所有階段維持手動工作的優先權。
...
- chore: CEO 開發盤點改為每小時一次、只產生建議；production 的產品自動分析維持預設關閉。
...
- fix: 將手動與排程工作分級，前端可追蹤目前節點、佇列位置、錯誤與完成報告。
- fix: Hermes 排程只每 30 分鐘建立低優先工作，worker 仍持續消化手動工作。

## v0.16.10 — 2026-07-18

- fix: 將前端手動分析與 Hermes 背景排程分離；手動 run 直接執行並回傳報告，不再等待快照佇列。
