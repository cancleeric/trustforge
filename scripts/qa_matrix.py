            ...
            try:
                if qtype == QuestionType.COMPARISON:
                    ...
                    report_a, evidence_a, report_b, evidence_b, log = run_comparison(
                        ...
                        run_scope_id=f"qa-matrix-{coin}-{time.time_ns()}",
                    )
                    ...
                    entry["coin_b"] = coin_b
                    # Collect failed sources from limits
...
def main(offline: bool = False, data_dir: str | None = None, out_dir: str = "out") -> int:
    """Entry point for the QA matrix script."""
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="QA mini matrix：5 幣 × 3 題型退化檢查")
    parser.add_argument("--offline", action="store_true", help="使用離線樣本資料")
    parser.add_argument("--data-dir", default=None, help="OHLCV 資料目錄")
    parser.add_argument("--out", default="out", help="輸出目錄")
    args = parser.parse_args()
    sys.exit(main(offline=args.offline, data_dir=args.data_dir, out_dir=args.out))
