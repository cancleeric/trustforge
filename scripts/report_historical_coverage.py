...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--latest-out", type=Path,
        default=REPO / "out" / "historical-coverage-latest.json",
        help="Canonical artifact consumed by the continuous outer-framework diagnostic",
    )
    ...
    if args.latest_out != args.out:
        args.latest_out.parent.mkdir(parents=True, exist_ok=True)
        args.latest_out.write_text(rendered, encoding="utf-8")
    summary = {
        ...
        "out": str(args.out), "latest_out": str(args.latest_out),
    }
