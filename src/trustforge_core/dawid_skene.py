"""Pure Dawid-Skene EM estimator for deterministic source consensus.

This module is the canonical implementation.  It deliberately depends only on
the Python standard library so applications can use the estimator without
loading TrustForge ingestion, providers, storage, or orchestration.

W2 動態信譽的離線 fallback（TrustForge #180/#182）。
