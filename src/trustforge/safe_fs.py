        ...
        try:
            if immutable:
                ...
                if existing is not None:
                    ...
                    backup = _temporary_name(f"{path.name}.backup")
                    os.link(path.name, backup, src_dir_fd=parent_fd, dst_dir_fd=parent_fd)
            ...
            try:
                ...
                if immutable:
                    ...
                    if backup is not None:
                        os.replace(backup, path.name, src_dir_fd=parent_fd, dst_dir_fd=parent_fd)
                        backup = None
                    else:
                        os.unlink(path.name, dir_fd=parent_fd)
                raise
