...
PCMDID=$(aws ssm send-command --region "$REGION" --instance-ids "$TARGET" \
  --document-name AWS-RunShellScript --parameters commands='["set -e","dnf install -y python3.11 python3.11-pip","/usr/bin/python3.11 -m pip install '\''boto3>=1.34'\'' '\''certifi>=2024.2.2'\'' '\''cryptography>=44,<50'\'' '\''portalocker>=3,<4'\'' '\''pypdf>=5,<7'\''","sed -i \"s|^ExecStart=/usr/bin/python3 -m trustforge.web$|ExecStart=/usr/bin/python3.11 -m trustforge.web|\" /etc/systemd/system/trustforge.service","for unit in fetch-scheduler.service hermes-cycle.service trustforge-analysis-flow.service; do if [ -f /etc/systemd/system/$unit ]; then sed -i \"s|/usr/bin/python3 |/usr/bin/python3.11 |g\" /etc/systemd/system/$unit; fi; done","systemctl daemon-reload","/usr/bin/python3.11 -c \"import boto3, certifi, cryptography, enum, portalocker, pypdf; assert hasattr(enum, '\''StrEnum'\'')\""]' \
  --query 'Command.CommandId' --output text)
