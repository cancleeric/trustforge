...
if [ "$MATCH_COUNT" -gt 1 ]; then
  ...
  rid=$(aws ssm send-command --region "$REGION" --instance-ids "$IID" \
    --document-name AWS-RunShellScript --parameters commands='["set -e","cd /opt/trustforge","bash /opt/trustforge/scripts/reconcile_trustforge_unit.sh","systemctl daemon-reload","bash deploy/zero_downtime_restart.sh"]' \
    --query 'Command.CommandId' --output text)
