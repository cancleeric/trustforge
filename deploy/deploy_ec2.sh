...
if [ "$BEDROCK_RPS_BACKEND" != "dynamodb" ]; then
...
EXTRA_UNIT_ENV=""
append_unit_env() {
  printf -v EXTRA_UNIT_ENV '%s%s\n' "$EXTRA_UNIT_ENV" "$1"
}

append_unit_env "Environment=TRUSTFORGE_TRAINING_DATA_DIR=${TRAINING_DATA_DIR}"
append_unit_env "Environment=TRUSTFORGE_WHALE_ALERT_SSM_PARAMETER=${WHALE_ALERT_SSM_PARAMETER}"
[ -n "$DAILY_CAP" ] && append_unit_env "Environment=TRUSTFORGE_BEDROCK_DAILY_USD_CAP=${DAILY_CAP}"
[ -n "$TOKEN_SSM_PREFIX" ] && append_unit_env "Environment=TRUSTFORGE_TOKEN_SSM_PREFIX=${TOKEN_SSM_PREFIX}"
...
for key in "${PREVIEW_ENV_KEYS[@]}"; do
  value="${!key-}"
  [ -n "$value" ] && append_unit_env "Environment=${key}=${value}"
done
