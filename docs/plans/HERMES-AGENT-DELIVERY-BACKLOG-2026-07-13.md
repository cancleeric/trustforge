...
| H-20 | Connector reliability policy | **Policy 與首輪 online observation 完成，429 cooldown 待 release。** `v0.14.3` cycle 成功寫入 32 個目標、131 筆文件，但 CryptoSlate 1、CoinGecko 1、Reddit 6 個目標收到 HTTP 429；流程安全降級並保留快取。已新增來源內 cooldown：首個明確 429 後停止該來源本輪剩餘幣別真呼叫，未刷新目標仍完整列入 failures。下一步發布並觀察重複輪次失敗率；證據見 `docs/qa/PREFETCH-PARALLELISM.md` | 每來源 owner、憑證、quota、retry/backoff、failure budget、fallback 記錄可查 |
