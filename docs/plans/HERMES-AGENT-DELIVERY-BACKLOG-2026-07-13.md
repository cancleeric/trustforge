...
| H-20 | Connector reliability policy | **來源內 429 cooldown 已完成 production 驗證，provider-wide 協調待做。** `v0.14.4` cycle 中 Reddit BTC 成功取得 24 筆；ETH 首次 429 後 SOL/BNB/XRP 不再發出真呼叫，但仍全部列入 failures。完整 cycle 51.03 秒、五幣快照 5/5、exit 0。CoinGecko 顯示同 provider 的不同 source worker 仍可能各自碰 429，下一步建立 provider-wide cooldown 並觀察重複輪次失敗率；證據見 `docs/qa/PREFETCH-PARALLELISM.md` | 每來源 owner、憑證、quota、retry/backoff、failure budget、fallback 記錄可查 |
