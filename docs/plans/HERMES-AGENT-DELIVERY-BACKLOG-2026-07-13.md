...
| H-02 | 正式資料預取部署 | **v0.14.2 production audit 已觸發並發現 packaging bug，v0.14.3 修正待 release。** timer 已成功 active，但 cycle 的 `measure_quality` 讀不到 sample data：封裝後推導根目錄會落到 `/opt`，而 `hermes-cycle.service` 缺 `TRUSTFORGE_HOME=/opt/trustforge`。已補 env 與 regression test；帶同一 env 的 24 題題庫全綠。release 後必須觀察第一輪完成狀態與連續七天 archive/snapshot | 連續 7 天每輪 archive/snapshot 可查、來源失敗率與 freshness 有 evidence | AWS/runtime 觀測 |
...
| H-19 | Production durable lease backend | **AWS infrastructure 與真實 contention 驗證完成。** 2026-07-14 已確認 `trustforge-analyze-leases` 為 ACTIVE/PAY_PER_REQUEST、TTL=`ttl` ENABLED；`trustforge-ec2` 最小 policy 僅含 Get/Put/Delete；兩個真 DynamoDB backend contention/release/reacquire 全綠。並修正 provisioning 對已啟用 TTL 可重跑。現行 EC2 service 尚未載入 lease env，必須隨下一次受驗 release 由 deploy script 寫入並做 service-level canary，不能假稱已生效 | production release 後確認 `TRUSTFORGE_IDEMPOTENCY_LEASE_BACKEND=dynamodb` + table env、service-level contention test 全綠 |
