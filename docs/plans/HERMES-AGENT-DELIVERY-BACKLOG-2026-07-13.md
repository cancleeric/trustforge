...
| H-20 | Connector reliability policy | **來源內 cooldown 已在 production 驗證；CoinGecko provider-wide cooldown 已實作待 release。** `v0.14.4` cycle 中 Reddit BTC 成功取得 24 筆；ETH 首次 429 後 SOL/BNB/XRP 不再發出真呼叫，但仍全部列入 failures。完整 cycle 51.03 秒、五幣快照 5/5、exit 0。CoinGecko 現已用 provider 級可重入鎖原子化 throttle、HTTP 結果與 process cache；任一 source 首次 429 後，其餘 CoinGecko worker 不再發出 HTTP。循序與雙 worker 併發測試全綠；下一步 release 並觀察重複輪次失敗率 | 每來源 owner、憑證、quota、retry/backoff、failure budget、fallback 記錄可查 |
