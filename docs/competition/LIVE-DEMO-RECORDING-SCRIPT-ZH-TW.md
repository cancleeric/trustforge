...
| 01:10–02:10 | 保持執行中的進度區可見，指向目前 stage 與 queue position（若有）；若等待較久，母帶不停止，評審版可用時間碼跳切並標示實際等待時間。 | 「畫面會顯示目前執行階段與排隊狀態。完成後，我們再用 Execution Panel 核對五階段與 run id；失敗會留下狀態，不會假裝完成。」 | `執行中：current stage｜完成後：五階段 + run id` |
