...
- [ ] BTC／ETH／SOL／BNB／XRP 可選，分析角度與題目輸入可用；題型由系統依輸入自動推導，不宣稱有可手動選擇的題型控制。
...
- [ ] Source / Config 交件包已綁定欲展示的 commit SHA，README、`pyproject.toml`、`uv.lock`、部署／執行說明可開啟；若 repository 非公開，已事先確認評審帳號或展示環境具備存取權。
- [ ] 畫面、網址列、開發者工具與下載檔均沒有 token、API key、password、cookie、私人金鑰或內部主機名稱。
- [ ] 麥克風音量與游標高亮已測試；正式錄影全程保留系統時間或 OBS 時間碼。

## 建議示範題目

幣種選 `BTC`，選擇與題目相符的分析角度，輸入：
...
| 00:20–00:50 | 點「新增分析」進入分析頁。依序指向幣種、分析角度與題目欄位。 | 「這次選 BTC，搭配與題目相符的分析角度，再用自然語言要求系統整合多源訊號、說明一致程度與可能推翻結論的條件；題型會由系統依輸入自動推導。」 | `BTC｜分析角度｜題型自動推導｜不預設答案` |
| 00:50–01:10 | 貼上建議題目，慢速捲動讓全文可讀，再按「開始分析」。 | 「送出後建立的是一個新的分析 job。TrustForge 不會先放一份固定報告，也不會在證據不足時硬湊高信心結論。」 | `建立新 run｜不足則 abstain` |
| 01:10–02:10 | 保持執行中的進度區可見，指向目前 stage 與 queue position（若有）；若等待較久，母帶不停止，評審版可用時間碼跳切並標示實際等待時間。 | 「畫面會顯示目前執行階段與排隊狀態。完成後，我們再用 Execution Panel 核對五階段、run id 與資料快照；失敗會留下狀態，不會假裝完成。」 | `執行中：current stage｜完成後：五階段 + run id` |
...
| 05:55–06:30 | 切到已準備、且評審已獲授權存取的 GitHub repository／commit 頁籤，顯示 commit SHA、README、依賴 lock 與部署說明；不要開啟任何 `.env` 或 secrets 頁面。 | 「第四件 Source / Config 是 commit 綁定、評審可存取的 repository 交件包，不是假裝成伺服器下載檔。它包含原始碼、依賴鎖定與執行說明，只記錄環境變數名稱和秘密注入方式，不包含秘密值。」 | `Source / Config｜commit-bound｜reviewer-accessible｜zero secrets` |
...
- 新 run 若仍在執行，評審版可剪去中間等待；跳切前保留送出後的 current stage 畫面，跳切後立即顯示完成結果的 run id，並加字幕「同一筆新送出分析，實際等待 X 分 Y 秒」。run id 只在介面實際顯示後核對，不宣稱執行中畫面已有該欄位；未剪輯母帶需完整保存。
- 超時、失敗或證據不足時，不切換到另一個 run 冒充本次結果。旁白應說明終態，再以明確標示的「備援既有 run」展示介面能力。
- 某來源種類沒有有效資料時，照實展示 coverage／limits；不可口頭宣稱系統已抓到畫面上不存在的來源。
- 正式站若沒有 Source / Config runtime 頁面，使用 commit 綁定且評審可存取的 repository 頁籤；若 repository 非公開，錄影前確認授權，不可口頭宣稱它是公開 repository，也不可把本機資料夾或私有設定頁說成正式交付介面。
...
- [ ] 新 run 完成後，Final Report、Execution Panel 與下載片段的 run id 一致；執行中畫面只描述實際可見的 current stage／queue position，若有跳切，已標示實際等待時間。
