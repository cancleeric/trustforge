...
報價區間僅供內部產品定位，不可直接作為對外報價。每案送出正式報價前，需完成 CBO 成本模型並確認：

1. 診斷包人日上限、POC 人日上限、導入包專案人日與維運責任邊界。
2. 內部成本率、目標毛利率與最低毛利門檻；低於門檻需重新報價或縮 scope。
3. 第三方 API、資料授權、雲端運算、模型推論、客戶環境 access 與加急交付成本，皆為 pass-through 或另列，不含在基礎區間。
4. 客製 connector、live provider calls、歷史資料授權與 production handoff 必須逐項估價，不能用 POC 下限包住。
5. 未經 CBO 成本模型與 security/cost review 核准，不得對客戶承諾固定價格、固定交期或 live data 可用性。
