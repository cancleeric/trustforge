...
每個成功 fetch 同步保存 `fetch_duration_ms`。`scripts/connector_reliability_report.py` 將 Bronze 事件與 scheduler 的成功／失敗紀錄合併，逐來源輸出：抓取次數、文件量、空批次、最新資料年齡、重複內容比例、p50/p95 延遲、失敗率、連續成功次數，以及 freshness／latency SLO 判定。新鮮度略過不算成功嘗試，避免健康度被排程 skip 灌水。
