...
# ---------------------------------------------------------------------------
# HIGH 修正（codex 複審，PR #39，theme-replay 最後一條）：/analyze 的每一種
# HTML 回應——200 成功頁與 400/429/502 錯誤頁——都必須把渲染結果存進
# `_render_cache` 並用 rtok 做主題切換連結（見 web.py 的 `analyze_page()`
# helper）。舊版錯誤頁落回 `default_toggle_href`（帶著含 live/real 模式
# 參數的原始 /analyze?... 當 next），切主題會 302 導回原網址、瀏覽器重新
# GET、pipeline 重跑——502 尤其可能已消耗部分 Bedrock 成本，重跑等於重複
# 花錢。這裡「by construction」統一驗證：不論回應是 200/400/429/502，抓
# 出頁面裡的 theme toggle href 直接呼叫，斷言 run()/run_comparison() 呼叫
# 次數完全不變（不是「呼叫了但沒壞」，是結構上根本沒被呼叫到）。
# ---------------------------------------------------------------------------

_THEME_TOGGLE_HREF_RE = re.compile(
    r'class="tf-theme-toggle" href="([^"]+)"'
)


def _extract_theme_toggle_href(body_html: str) -> str:
    """從 render_page 輸出裡把 header「★」主題切換連結的 href 抓出來，
    html.unescape 還原成可直接拿去請求的真實路徑（模板用 html.escape 過，
    `&` 會變成 `&amp;`）。"""
    m = _THEME_TOGGLE_HREF_RE.search(body_html)
    assert m, f"body 裡找不到 tf-theme-toggle 連結：{body_html[:500]}"
    return html.unescape(m.group(1))


def _assert_toggle_href_uses_rtok_not_raw_analyze(href: str) -> str:
    """斷言主題切換連結走的是 rtok 快取重繪路徑，不是把原始
    `/analyze?...`（可能帶 live/real 模式參數）塞進 `next` 的舊路徑。回傳
    href 供呼叫端接著實際打 `/theme` 驗證不重跑 pipeline。"""
    parsed = urlparse(href)
    assert parsed.path == "/theme", f"主題切換連結應指向 /theme，實際：{href}"
    qs = parse_qs(parsed.query)
    assert "rtok" in qs and qs["rtok"][0], (
        f"/analyze 回應的主題切換連結必須帶 rtok（by construction 統一走渲染"
        f"快取重繪），不可落回帶 next=/analyze?... 的舊路徑：{href}"
    )
    assert "next" not in qs, (
        f"帶 rtok 就不該同時帶 next（代表誤走了 default_toggle_href 的 "
        f"fallback）：{href}"
    )
    return href


def test_theme_toggle_after_200_success_report_does_not_rerun_pipeline(monkeypatch):
    """成功頁（既有行為，回歸鎖定）：切主題走 rtok，不重跑 pipeline。"""
    monkeypatch.setattr(web, "HAS_BEDROCK", False)
    monkeypatch.setattr(web, "LIVE_TOKEN", "")
    web._rate_buckets.clear()

    result = _run_do_get(
        "/analyze?coin=BTC&type=multi_source&q=t", client_ip="10.0.1.1"
    )
    assert result["code"] == 200
    href = _assert_toggle_href_uses_rtok_not_raw_analyze(
        _extract_theme_toggle_href(result["body"])
    )
    ...
    toggled = _run_do_get(href)  # 若內部誤呼叫 run()，_boom 會在這裡 raise
    assert toggled["code"] == 200
    assert 'data-theme="light"' in toggled["body"]


def test_theme_toggle_after_400_error_page_does_not_rerun_pipeline(monkeypatch):
    """400 錯誤頁（幣種非法，?real=1）：切主題走 rtok，不重跑 pipeline。"""
    monkeypatch.setattr(web, "HAS_BEDROCK", False)
    monkeypatch.setattr(web, "LIVE_TOKEN", "")
    web._rate_buckets.clear()

    result = _run_do_get(
        "/analyze?coin=NOPE&type=multi_source&q=t&real=1", client_ip="10.0.1.2"
    )
    assert result["code"] == 400
    href = _assert_toggle_href_uses_rtok_not_raw_analyze(
        _extract_theme_toggle_href(result["body"])
    ...
    toggled = _run_do_get(href)
    assert toggled["code"] == 200
    assert 'data-theme="light"' in toggled["body"]
    # 錯誤頁重繪內容應與原錯誤頁一致（不是重新分析出的新內容）。
    actives = _ACTIVE_BADGE_RE.findall(toggled["body"])
    assert actives == ["tf-real"], f"切主題後應仍是同一份 400 tf-real 錯誤頁，實際：{actives}"


def test_theme_toggle_after_429_error_page_does_not_rerun_pipeline(monkeypatch):
    """429 錯誤頁（live 限流超速）：切主題走 rtok，不重跑 pipeline、不重複計入限流。"""
    monkeypatch.setattr(web, "HAS_BEDROCK", True)
    monkeypatch.setattr(web, "LIVE_TOKEN", "secret")
    web._rate_buckets.clear()
    ip = "10.0.1.3"
    ...
    # 用另一個未被灌爆的 IP 打 /theme，確認就算限流桶還是滿的，重繪走的是
    # 快取而非重新呼叫 _check_live_rate_limit/_do_analyze。
    toggled = _run_do_get(href, client_ip=ip)
    assert toggled["code"] == 200
    assert 'data-theme="light"' in toggled["body"]
    actives = _ACTIVE_BADGE_RE.findall(toggled["body"])
    assert actives == ["tf-live"], f"切主題後應仍是同一份 429 tf-live 錯誤頁，實際：{actives}"


def test_theme_toggle_after_502_error_page_does_not_rerun_pipeline(monkeypatch):
    """502 錯誤頁（pipeline 內部丟未預期例外，?real=1）：切主題走 rtok，不
    重跑 pipeline——這是本輪修正最關鍵的案例：502 常代表已經打了部分
    Bedrock 呼叫後才失敗，重跑等於重複花真金錢。"""
    monkeypatch.setattr(web, "HAS_BEDROCK", False)
    monkeypatch.setattr(web, "LIVE_TOKEN", "")
    web._rate_buckets.clear()
    ...
    result = _run_do_get(
        "/analyze?coin=BTC&type=multi_source&q=t&real=1", client_ip="10.0.1.4"
    )
    assert result["code"] == 502
    assert call_count["n"] == 1, "502 錯誤頁本身應該只呼叫一次 run()（本次失敗那次）"
    href = _assert_toggle_href_uses_rtok_not_raw_analyze(
        _extract_theme_toggle_href(result["body"])
    )
    ...
    assert call_count["n"] == 1, (
        "切主題後 run() 呼叫次數不應增加——502 錯誤頁必須從 _render_cache "
        "重繪，不能觸發瀏覽器重新 GET /analyze?...real=1 而重跑一次 "
        "pipeline（等於重複花費已經部分發生的 Bedrock 成本）"
    )
    actives = _ACTIVE_BADGE_RE.findall(toggled["body"])
    assert actives == ["tf-real"], f"切主題後應仍是同一份 502 tf-real 錯誤頁，實際：{actives}"
