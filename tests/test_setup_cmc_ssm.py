...
SCRIPT = (
    Path(__file__).resolve().parents[1] / "deploy" / "setup_cmc_ssm.sh"
)


def test_cmc_policy_is_least_privilege_exact_parameter():
    """CoinMarketCap IAM policy 鎖在單一 exact SSM parameter ARN、6 個最小 ssm op，
    ...
    # (d) Resource 是單一 exact ARN（str 而非 list，且指向唯一 cmc parameter）。
    assert isinstance(statement["Resource"], str)
    assert statement["Resource"].endswith(
        ":parameter/trustforge/production/cmc-api-key"
    )

    # (b) Resource 不可含 wildcard（exact parameter only，不可 "arn:aws:ssm:*:*:*"）。
...
def _run_with_parameter(parameter: str) -> subprocess.CompletedProcess:
    env = {**os.environ, "TRUSTFORGE_CMC_SSM_PARAMETER": parameter}
...
def test_cmc_rejects_trailing_dotdot_segment():
    """codex P2：拒任何 `..` path segment（與 cmc_secret._parameter_name() 一致）。
...
def test_cmc_rejects_mid_dotdot_segment():
...
def test_cmc_accepts_normal_parameter_without_dotdot():
    """正常參數名（無 `..` segment）仍通過——回歸保護，確認新檢查沒誤殺。"""
    result = _run_with_parameter("/trustforge/production/cmc-api-key")
    assert result.returncode == 0
