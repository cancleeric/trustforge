    ...
    class Executor:
        ...
        def execute(self, bound):
            ...
            return SimpleNamespace(
                outcome=outcome,
                handle=object() if outcome is AdmissionOutcome.ADMITTED else None,
            )
...
def _plane(outcome=AdmissionOutcome.DENIED, tokenizer=None, planner=None):
    ...
    plane = HermesPreviewControlPlane(
        ...
        proxy_policy=TrustedProxyPolicy(("127.0.0.0/8", "10.0.0.0/8")),
        observer=observer,
    )
...
def test_policy_is_fixed_and_silent_cap_relaxation_is_rejected():
    ...
    with pytest.raises(ValueError, match="invalid preview policy"):
        _policy(reserved_micro_usd=50_001)


def test_proxy_identity_requires_trusted_peer_and_canonical_ip():
    policy = TrustedProxyPolicy(("127.0.0.0/8",))
...
def test_stale_price_policy_fails_before_executor():
    ...
    plane = HermesPreviewControlPlane(
        ...
        tokenizer=Tokenizer(),
        planner=Planner(),
        proxy_policy=TrustedProxyPolicy(("127.0.0.0/8",)),
        observer=observer,
    )
    result = plane.admit(
        _request(), peer_ip="127.0.0.1", canonical_client_ip="203.0.113.2"
...
def test_schema_failure_is_uncertain_worst_case_and_not_circuit(monkeypatch):
    planner = Planner(PlannerPortFailure(PreviewTerminalClass.SCHEMA_FAILURE))
