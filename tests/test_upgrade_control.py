...
def test_upgrade_control_exposes_six_versioned_modules_without_recursive_apply(monkeypatch, tmp_path):
    ...
    assert next(m for m in data["modules"] if m["id"] == "core")["state"] == "locked"
    assert next(m for m in data["modules"] if m["id"] == "verify")["state"] == "candidate"
